...
function buildTaxonomy(
  ...
  tagProfile = buildItemTagProfile(detail, detail.statistic),
): BoardTaxonomy {
  const text = `${detail.marketHashName ?? detail.name} ${detail.weapon ?? ""}`.toLowerCase();

  if (tagProfile.itemTypeKey === "sticker" || includesAny(text, ["sticker", "印花"])) {
    const sticker = resolveStickerSegment(`${detail.marketHashName ?? detail.name} ${detail.name}`);
    return {
      categoryKey: "sticker",
      categoryLabel: "贴纸板块",
      segmentKey: tagProfile.stickerSeriesKey ?? sticker.segmentKey,
      segmentLabel: tagProfile.stickerSeriesLabel ?? sticker.segmentLabel,
      spotlight: sticker.spotlight,
    };
  }

  if (
    tagProfile.itemTypeKey === "glove" ||
    includesAny(text, ["glove", "手套", "hand wraps", "bloodhound", "hydra", "broken fang"])
  ) {
  ...
  if (tagProfile.itemTypeKey === "agent" || includesAny(text, ["agent", "探员", "特工"])) {
    return {
      categoryKey: "agent",
      categoryLabel: "探员板块",
      segmentKey: tagProfile.originKey === "operation_agent" ? "operation_agent" : "agent_generic",
      segmentLabel: tagProfile.originKey === "operation_agent" ? "大行动探员" : "探员饰品",
      spotlight: "探员更像题材轮动品种，仓位集中、来源稀缺度和行情热度会更关键。",
    };
  }
  ...
  if (
    ...
    detail.weapon ||
    includesAny(text, ["ak-47", "m4a", "awp", "glock", "usp", "desert eagle"])
  ) {
    if (tagProfile.recommendationScopes.includes("discontinued_collection_skin")) {
      return {
        categoryKey: "gun",
        categoryLabel: "枪皮板块",
        segmentKey: "discontinued_collection_skin",
        segmentLabel: "绝版收藏品枪皮",
        spotlight: "绝版收藏品枪皮更看重来源稀缺、存世量区间、磨损和高品质权重的共振。",
      };
    }
...
function evaluateAutonomousRecommendationEligibility(
...
export function isAutonomousRecommendationEligible(
  analysis: AnalysisResponse,
  scanner?: Pick<ScannerConfig, "analysisScopes" | "holoStickerSeries"> | null,
) {
  return evaluateAutonomousRecommendationEligibility(analysis, scanner).passed;
}
...
function buildBottomReversalSignal({
  ...
  tagProfile: AnalysisResponse["tagProfile"];
  ...
  const eligibleBoard =
    (taxonomy.categoryKey === "gun" &&
      ...
      currentStatistic <= 40_000 &&
      tagProfile.recommendationScopes.some((scope) =>
        scope === "gun_skin" || scope === "discontinued_collection_skin"
      )) ||
    (taxonomy.categoryKey === "glove" &&
      currentStatistic != null &&
      currentStatistic >= 2_000 &&
      currentStatistic <= 7_000) ||
    tagProfile.recommendationScopes.includes("agent") ||
    (taxonomy.categoryKey === "sticker" &&
      tagProfile.recommendationScopes.includes("holo_team_sticker") &&
      isHoloStickerName(nameText));
...
export function buildRecommendationCard(
  ...
  return {
    ...
    taxonomy: analysis.taxonomy,
    tagProfile: {
      itemTypeKey: analysis.tagProfile.itemTypeKey,
      itemTypeLabel: analysis.tagProfile.itemTypeLabel,
      weaponClassKey: analysis.tagProfile.weaponClassKey,
      rarityLabel: analysis.tagProfile.rarityLabel,
      specialKey: analysis.tagProfile.specialKey,
      wearKey: analysis.tagProfile.wearKey,
      stickerFinishKey: analysis.tagProfile.stickerFinishKey,
      originLabel: analysis.tagProfile.originLabel,
      sourceSeriesLabel: analysis.tagProfile.sourceSeriesLabel,
      stickerSeriesLabel: analysis.tagProfile.stickerSeriesLabel,
      supplyBandKey: analysis.tagProfile.supplyBandKey,
      recommendationScopes: analysis.tagProfile.recommendationScopes,
    },
    hypeFitScore: analysis.tagProfile.hypeFitScore,
    hypeTags: analysis.tagProfile.hypeTags,
    recommendationType,
...
export function buildRecommendationResponse(
  ...
  const cards = analyses
    .map((analysis) => buildRecommendationCard(analysis, options?.scanner))
  ...
  const positive = cards
    ...
    .sort(
      (left, right) =>
        ...
        right.hypeFitScore - left.hypeFitScore ||
...
export async function analyzeItem(
  ...
  const tagProfile = buildItemTagProfile(detail, currentStatistic);
  const taxonomy = buildTaxonomy(detail, latestBlend, tagProfile);
