...
export class CsqaqClient {
  ...
  async getSubData(indexId: string | number, type = "daily"): Promise<MarketIndexDetail> {
    const payload = await this.request<{
      count?: Record<string, unknown>;
    }>(
      `/sub_data?id=${encodeURIComponent(String(indexId))}&type=${encodeURIComponent(type)}`,
      { method: "GET" },
    );

    const count = payload.data.count ?? {};
    return {
      now: nullableNumber(count.now),
      amplitude: nullableNumber(count.amplitude),
      rate: nullableNumber(count.rate),
      maxValue: nullableNumber(count.max_value),
      minValue: nullableNumber(count.min_value),
      consecutiveDays: nullableNumber(count.consecutive_days),
    };
  }

  async getSubKline(indexId: string | number, type = "1day"): Promise<MarketIndexCandle[]> {
    const payload = await this.request<Array<Record<string, unknown>>>(
      `/sub/kline?id=${encodeURIComponent(String(indexId))}&type=${encodeURIComponent(type)}`,
      { method: "GET" },
    );

    return (Array.isArray(payload.data) ? payload.data : [])
      .map<MarketIndexCandle>((row) => ({
        t: Number(row.t ?? 0),
        o: Number(row.o ?? 0),
        c: Number(row.c ?? 0),
        h: Number(row.h ?? 0),
  ...
  async getPageList(pageIndex = 1, pageSize = 24, options: PageListOptions = {}) {
    const payload = await this.request<{
  ...
  async getContainerDataInfo() {
    const payload = await this.request<Array<Record<string, unknown>>>(
      "/info/container_data_info",
      {
        method: "POST",
        body: JSON.stringify({}),
      },
  ...
  async getContainerDetail(containerId: string | number) {
    const payload = await this.request<Array<Record<string, unknown>>>(
      `/info/good/container_detail?id=${encodeURIComponent(String(containerId))}`,
      { method: "GET" },
    );
    ...
    return (payload.data ?? []).map((row) => ({
      ...
      rarity: typeof row.rln === "string" ? row.rln : null,
      quality: typeof row.qln === "string" ? row.qln : null,
    }));
  }
  ...
  async getMonitorTaskInventory(taskId: string | number, pageIndex = 1, pageSize = 24) {
    ...
    return (payload.data ?? []).map<MonitorInventoryItem>((row) => {
      const tradableValue = String(row.tradable ?? "").toLowerCase();
      const tradable = row.tradable === true || tradableValue === "1" || tradableValue === "true";
      const parsedCount = Number(row.num ?? row.count ?? 1);
      ...
      return {
        ...
        tradable,
        count,
        tradableCount: tradable ? count : 0,
        cooldownCount: tradable ? 0 : count,
